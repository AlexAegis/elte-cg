...
void CMyApp::DrawGround()
{
	...
	*/
	...
	glUniform1i(	m_loc_texture,	// ezen azonosítójú sampler 2D
		0 /*hw samplerindex*/);		// olvassa az ezen indexű mintavételezőt
...
void CMyApp::Render()
{
    ...
    ImGui::SliderInt("Integer:", &egesz, 1, 15);
    ImGui::SliderFloat("Angle:", &angle, 0, 6.28);
    
    

    ImGui::Text("Eye:\n x: %f, y: %f, z: %f",m_eye.x,m_eye.y,m_eye.z);
	ImGui::Text("At:\n x: %f, y: %f, z: %f", m_at.x, m_at.y, m_at.z);
	ImGui::Text("Up:\n x: %f, y: %f, z: %f", m_up.x, m_up.y, m_up.z);


    ImGui::Checkbox("GrafCheckbox", &check);

        ImGui::RadioButton("Choice #1", &radioButton, 0); ImGui::SameLine();
        ImGui::RadioButton("Choice #2", &radioButton, 1); ImGui::SameLine();
        ImGui::RadioButton("Choice #3", &radioButton, 2);
    
//    printf("Integer:%d %d %d %d\n",egesz,check,radioButton,clicked);
	...
	ImGui::Text("ChkBox:%d Selection:%d ClickCount: %d", check, radioButton, clicked);
}
