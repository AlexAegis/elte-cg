...
GLuint CMyApp::GenTexture()
{
	...
	glGenerateMipmap(GL_TEXTURE_2D);	//KOMMENT: mipmap elmagyarázása

	//KOMMENT: bilineáris szûrés kicsinyítéskor
    glTexParameteri(GL_TEXTURE_2D,	//KOMMENT: melyik binding point-on van a textúra erõforrás
		GL_TEXTURE_MIN_FILTER,		//KOMMENT: ha egy pixel területére több textúra elem is esne
		GL_NEAREST);				//KOMMENT: --> a vizsgált helyhez (pixel közepe) legközelebb esõ textúra elemet adja vissza
	// és nagyításkor is
	glTexParameteri(GL_TEXTURE_2D,
		GL_TEXTURE_MAG_FILTER,		//KOMMENT: ha egy pixel területe kisebb (v egyenlõ), mint az "alá esõ" textúra elem
		GL_NEAREST);				//KOMMENT: --> a vizsgált helyhez (pixel közepe) legközelebb esõ textúra elemet adja vissza
...
void CMyApp::Render()
{
	...
	*/
	...
	glActiveTexture(GL_TEXTURE0);	//KOMMENT: implementációfüggõen változó mennyiségû ilyen egység lehet (de minimum 2 mindig van)
	...
	glUniform1i(	m_loc_texture,	// ezen azonosítójú sampler 2D
					0);				// olvassa az ezen indexû mintavételezõt //KOMMENT: 0 ==> GL_TEXTURE0


	//1.:
	glUniform2f(m_offset, (SDL_GetTicks() % 1000) / 1000.0, (SDL_GetTicks() % 2500) / 2500.0);
