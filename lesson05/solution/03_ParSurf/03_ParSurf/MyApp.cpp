...
glm::vec3	CMyApp::GetUV(float u, float v)
{
	...
	/*
	u *= 2*3.1415f;	//KOMMENT: mivel 0 <= u,v <= 1
					//		--> nekünk pedig ez kell:
					//									-PI <= u <= PI
					//									0 <= v <= PI
					//          --> EZÉRT: a (0,2*PI), illetve (0,PI) intervallumra transzformáljuk õket
					//				--> így az "u" és "v" szögek pontosan egy kört járnak be...
					// KOMMENT: OK: mivel egy félkört (180 fok) forgatunk körbe (360 fok)
	v *= 3.1415f;
	float cu = cosf(u), su = sinf(u), cv = cosf(v), sv = sinf(v);
	float r = 2;	//KOMMENT: a kör sugara

	return glm::vec3( r*cu*sv, r*cv, r*su*sv );
	*/
	
	//KOMMENT: theta <==> v      phi <==> u



	//1.:
	//KOMMENT: egy kört (360 fok) forgatunk körbe (360 fok)
	u *= 2 * 3.1415f;
	v *= 2 * 3.1415f;
	float cu = cosf(u), su = sinf(u), cv = cosf(v), sv = sinf(v);
	float R = 2;	//KOMMENT: a kör sugara --> a henger "közepén levõ" kör sugara
	float r = 1;	//KOMMENT: a henger sugara
...
bool CMyApp::Init()
{
	...
	//KOMMENT: egy sík beli egység noldalú négyzetrácsot gömb alakúra hajtogatunk...
	Vertex vert[(N+1)*(M+1)];
	for (int i=0; i<=N; ++i)
		for (int j=0; j<=M; ++j)
		{
			float u = i/(float)N;	//KOMMENT: így: u <eleme> (0, 1/N, 2/N, ... , N/N)
			float v = j/(float)M;	//KOMMENT: így: v <eleme> (0, 1/M, 2/M, ... , M/M)
	...
	for (int i=0; i<N; ++i)
		for (int j=0; j<M; ++j)
		{
			...
			// - az (i,j)-hez tartózó 1D-s index az IB-ben: i*6+j*6*(N+1) //KOMMENT: NEM "(N+1)", hanem csak "(N)"
