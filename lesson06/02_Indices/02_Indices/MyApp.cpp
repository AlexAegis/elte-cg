...
void CMyApp::Update()
{
	...
	//m_matView = glm::lookAt(glm::vec3( 10 * sinf(t),  10,  5 * cosf(t)),		// honnan nézzük a színteret
		m_matView = glm::lookAt(glm::vec3(15, 15, 15),
...
void CMyApp::Render()
{
	...
	if (!stopped) {
		t = (SDL_GetTicks() / 1000.0f) - difference;
	...
	for (int i = 0; i < 12; i++) {
		...
		*/
		...
		m_matWorld = glm::rotate<float>(2 * M_PI * t / 6, glm::vec3(1, 1, 0)) * glm::translate<float>(glm::vec3(8.0f, 0, 0)) * glm::rotate<float>(2 * M_PI * t / 6, glm::vec3(1, 1, 0));
		m_matWorld = glm::rotate<float>(2 * M_PI * i / 12, glm::vec3(1, 1, 0)) * m_matWorld;
...
// kapcsoljuk be a VAO-t (a VBO jön vele együtt)
		...
		glDrawElements(GL_TRIANGLES,		// primitív típus
			18,					// hany csucspontot hasznalunk a kirajzolashoz
...
void CMyApp::KeyboardDown(SDL_KeyboardEvent& key)
{
	if(key.keysym.sym == SDLK_SPACE) {
		if (!stopped) {
			stopped_at = SDL_GetTicks() / 1000.0f;
		}
		else {
			difference = difference + SDL_GetTicks() / 1000.0f - stopped_at;
		}
		stopped = !stopped;
