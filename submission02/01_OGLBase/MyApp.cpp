...
bool CMyApp::Init()
{
	...
	for (int i = 0; i < N; ++i) {
		for (int j = 0; j < M; ++j)
		{
			indices[6 * i + j * 6 * (N)+0] = (i)+(j)   * (N + 1);
			indices[6 * i + j * 6 * (N)+1] = (i + 1) + (j)   * (N + 1);
			indices[6 * i + j * 6 * (N)+2] = (i)+(j + 1) * (N + 1);
			indices[6 * i + j * 6 * (N)+3] = (i + 1) + (j)   * (N + 1);
			indices[6 * i + j * 6 * (N)+4] = (i + 1) + (j + 1) * (N + 1);
			indices[6 * i + j * 6 * (N)+5] = (i)+(j + 1) * (N + 1);
		}
	}
	...
	*/
	m_gpuBufferPos.BufferData(vertVector);
