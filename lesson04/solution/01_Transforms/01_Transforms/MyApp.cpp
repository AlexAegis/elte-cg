...
bool CMyApp::Init()
{
	...
	//0.2.:
	//KOMMENT: hátulról nézett háromszögek éleinek kirajzolásas
...
void CMyApp::Render()
{
	...
	*/
	...
	//0.1.:
	m_matWorld = glm::rotate<float>( SDL_GetTicks()/300.0, glm::vec3(1,0,0) ) * m_matWorld;
	//
	m_matWorld = glm::translate<float>(glm::vec3(sin(SDL_GetTicks() / 300.0), 0, 0)) * m_matWorld;
	//
	float my_scale = 2.0 + sin(SDL_GetTicks() / 600.0);
	m_matWorld = glm::scale<float>(glm::vec3(my_scale, my_scale, 1)) * m_matWorld;
