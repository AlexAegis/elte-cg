...
//4.:
void CMyApp::generate_rand(std::vector<glm::vec3> &vec, float mult)
{
	...
	{
		vec[i] = glm::vec3(mult * rnd(gen), mult * rnd(gen), mult * rnd(gen));
	}
}
...
bool CMyApp::Init()
{
	...
	//3.:
	m_velocityProgram.Init({
		{ GL_VERTEX_SHADER,		"velocity.vert" },
		{ GL_FRAGMENT_SHADER,	"velocity.frag" }
	},
	{
		{ 0, "vs_in_pos" },
	});
	...
	*/
	...
	m_particlePos.reserve(m_particleCount);	//KOMMENT: legyen elég hely a részecskék adatainak eltárolásához
	m_particleVel.reserve(m_particleCount);
	//3.:
	m_particleVelArrows.reserve(m_particleCount * 2);
	...
	*/
	m_particlePos.resize(m_particleCount);
	m_particleVel.resize(m_particleCount);
	generate_rand(m_particlePos, 1.0);
	generate_rand(m_particleVel, 2.0);
	...
	//3.:
	m_particleVelArrows.resize(m_particleCount * 2);
	m_gpuVelArrowBuffer.BufferData(m_particleVelArrows);
	m_gpuVelArrowVAO.Init({
		{ CreateAttribute<0, glm::vec3, 0, sizeof(glm::vec3)>, m_gpuVelArrowBuffer }
	});
...
void CMyApp::Update()
{
	...
	for (int i = 0; i < m_particleCount; ++i)
	{
		...
		//1.:
		float falling_acceleration = particle_weight * gravity * delta_time;
		if (m_particlePos[i].y <= -1 && m_particleVel[i].y - falling_acceleration <= 0)
			m_particleVel[i].y = 0.0;
		else
			m_particleVel[i].y -= particle_weight * gravity * delta_time;
		
		//2.:
		for (int j = 0; j < m_particleCount; ++j) {
			glm::vec3 v = glm::vec3(
				m_particlePos[i].x - m_particlePos[j].x,
				m_particlePos[i].y - m_particlePos[j].y,
				m_particlePos[i].z - m_particlePos[j].z
			);
			float distance = sqrt(pow(v.x, 2) + pow(v.y, 2) + pow(v.z, 2));
			if (distance < bounce_threshold && j != i) {
				glm::vec3 swap_buffer = m_particleVel[i];
				m_particleVel[i] = m_particleVel[j];
				m_particleVel[j] = swap_buffer;
			}
		}

		//3.:
		m_particleVelArrows[2 * i] = m_particlePos[i];
		m_particleVelArrows[2 * i + 1] = m_particleVel[i];
	}
	...
	//3.:
	glBindBuffer(GL_ARRAY_BUFFER, m_gpuVelArrowBuffer);
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3)*m_particleVelArrows.size(), &(m_particleVelArrows[0][0]));
...
void CMyApp::Render()
{
	...
	//3.:
	m_gpuVelArrowVAO.Bind();
	m_velocityProgram.Use();
	m_velocityProgram.SetUniform("mvp", m_camera.GetViewProj());
	...
	{
		//0.:
		ImGui::SliderFloat("Energy remaining", &energyRemaining, 0.0, 2.0);
		...
		if (ImGui::Button("Add"))
		{
			...
			m_particlePos.resize(m_particleCount + 1);
			m_particleVel.resize(m_particleCount + 1);

			m_particlePos[m_particleCount] = glm::vec3(rnd(gen), rnd(gen), rnd(gen));
			m_particleVel[m_particleCount] = glm::vec3(2 * rnd(gen), 2 * rnd(gen), 2 * rnd(gen));
			
			m_particleVelArrows.push_back(m_particlePos[m_particleCount]);
			m_particleVelArrows.push_back(m_particleVel[m_particleCount]);
		...
		if (ImGui::Button("Remove") && m_particleCount > 1)
		{
			m_particlePos.pop_back();
			m_particleVel.pop_back();

			m_particleVelArrows.pop_back();
			m_particleVelArrows.pop_back();
