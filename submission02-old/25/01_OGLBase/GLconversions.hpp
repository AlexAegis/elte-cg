...
template <typename T>			constexpr const T* PointerToStart(const std::vector<T>& pArg) { return &pArg[0]; }
...
template<typename T, size_t N> struct HasContiguousStorage<typename T[N]> : public std::true_type {};

template <typename T, typename R = void> 
using IsContiguousContainer = std::enable_if_t<HasContiguousStorage<typename T>::value, R>;